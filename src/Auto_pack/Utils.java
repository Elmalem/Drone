...
public class Utils {
	...
	public static void updateInfo(int deltaTime, JLabel info_label, JLabel info_label2, Algorithm algo) {
		...
		info_label2.setText("<html>" + String.valueOf(algo.counter) + " <BR>isRisky:" + String.valueOf(algo.is_risky)
				+ "<BR>" + String.valueOf(algo.risky_dis) + "<BR> Clock : " + String.valueOf(Config.dtf.format(now))
				+ "<BR> Time : " + String.valueOf(CPU.getElapsedMilli() / 1000) + "</html>");
	}
	
	public static void updateTime(int deltaTime , JLabel info) {
		info.setText("<html>" + "<BR> Time : " + String.valueOf(CPU.getElapsedMilli() / 1000) + "</html>");
	}
	...
	public static void ai(int deltaTime, Algorithm algo) {
		...
		if (Simulator.return_home) {
			...
			if (Utils.getDistanceBetweenPoints(Utils.getLastPoint(algo), dronePoint) < 100) {
				...
				if (algo.points.size() <= 1
						&& Utils.getDistanceBetweenPoints(Utils.getLastPoint(algo), dronePoint) < 20) {
		...
		if (!algo.is_risky) {
			for (int i = 0; i < algo.drone.lidars.size(); i++) {
				...
				switch (i) {
				case 0:
					if (lidar.current_distance <= Config.max_risky_distance) {
						algo.is_risky = true;
				...
				case 1:
				case 2:
					if (lidar.current_distance <= Config.max_risky_distance / 3) {
						algo.is_risky = true;
					}
					break;
				}
		...
		// Unrisky case
			...
			if (!algo.try_to_escape) {
				...
				if (right_lidar_current_dis > 270 && left_lidar_current_dis > 270) {
					...
					Point l1 = Utils.getPointByDistance(dronePoint, lidar1.degrees + algo.drone.getGyroRotation(),
							lidar1.current_distance);
					Point l2 = Utils.getPointByDistance(dronePoint, lidar2.degrees + algo.drone.getGyroRotation(),
					...
					// If my left distance grater than the right one spin by opposite
					if (right_lidar_current_dis < left_lidar_current_dis || algo.risky_dis >= 100) {
						spin_by *= (-1);
					}
				}

				if ((right_lidar_current_dis <= 1 && left_lidar_current_dis <= 1 && forward_lidar_current_dis <= 1)
						&& (dronePoint.x > 1 && dronePoint.y > 1)) {
